...
export async function getSubcategoryData({slug, page = 1, cat, tag}) {
    let url = `${API_URL}/subcategory/${slug}?page=${page}&cat[]=${Array.isArray(cat) ? cat.join("&cat[]=") : cat}${tag ? `&tag[]=${tag}` : ``}`;
...
export async function getSearchData({q, page = 1, cat, tag}) {
    let url = `${API_URL}/search?q=${encodeURIComponent(q)}&page=${page}&cat[]=${Array.isArray(cat) ? cat.join("&cat[]=") : cat}${tag ? `&tag[]=${tag}` : ``}`;
    url = url.replace("cat[]=0", "archive_heading=1");
